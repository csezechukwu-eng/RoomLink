...
const DEMO_PROPERTY_CONFIG = {
  ...
  description:
    "Demo shared housing property used to test renta bed's dashboard, property, room, bed, application, and lease workflows.",
...
async function generateDemoLeasePdf(): Promise<Uint8Array> {
  ...
  // Title
  page.drawText("RENTA BED DEMO LEASE", {
    x: 50,
    y: height - 60,
    size: 24,
    font: helveticaBold,
  ...
  page.drawText("This is a demo lease for testing renta bed features.", {
    x: 50,
    y: height - 120,
    size: 11,
    font: helvetica,
  ...
  const content = [
    ...
    "",
    "LANDLORD: renta bed Demo Landlord",
    "PROPERTY: renta bed Demo House",
  ...
  // Footer
  page.drawText("Generated by renta bed Demo System", {
    x: 50,
    y: 40,
    size: 9,
    font: helvetica,
...
export async function checkFullDemoReadiness(): Promise<Result<FullDemoReadinessResult>> {
  try {
    ...
    const demoLeaseTemplate = allTemplates.find(
      (t) => t.title === "renta bed Demo Month-to-Month Lease"
    ) || null;
...
export async function seedFullDemoData(): Promise<Result<DemoSeedResult>> {
  try {
    ...
    const { data: existingTemplate } = await supabase
      ...
      .select("*")
      .eq("owner_id", ownerId)
      .eq("title", "renta bed Demo Month-to-Month Lease")
      .maybeSingle();
    ...
    if (existingTemplate) {
      ...
      const fileName = "renta-bed-demo-lease.pdf";
      ...
      const { data: template, error: tErr } = await supabase
        ...
        .insert({
          owner_id: ownerId,
          title: "renta bed Demo Month-to-Month Lease",
          ...
          notes: storageWarning
            ...
            : "Demo lease template for testing renta bed features.",
        })
        .select()
        .single();
    ...
    for (let i = 0; i < DEMO_APPLICANTS.length; i++) {
      ...
      const insertPayload = {
        ...
        reason_for_stay: "Demo application for testing renta bed workflows",
